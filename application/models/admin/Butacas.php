...

{
  ...
  function get_total_recaudado(){
    $this->db->select("*");
    $this->db->from('tbl_butaca');
    $this->db->where('estado','RT');
    $query = $this->db->get();
    $sum = 0;
    if ($query->num_rows()!=0){
  ...
  function asientos_List1(){
    ...
    $this->db->group_by('tbl_reserva.id_reserva');
  ...
  //Alumnos que no tienen entradas
  function asientos_List3(){

    $this->db->select('alumnos.documento');

    $this->db->from('alumnos');

     $this->db->join('tbl_reserva', 'tbl_reserva.dni_alumno = alumnos.documento', 'inner');

     $this->db->join('deta_reserva','deta_reserva.id_reserva = tbl_reserva.id_reserva', 'inner');
    ...
    $this->db->select('alumnos.apellido, alumnos.documento, alumnos.nombre ,alumnos.sexo');
    ...
    $this->db->where('alumnos.documento NOT IN ('.$subquery.')',NULL,FALSE);
